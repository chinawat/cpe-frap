...
attribute [local simp]
  aeval beval aequiv bequiv cequiv update
...
def precondition_from (dec : Decorated) : Assertion :=
  match dec with
  | decorated P _ => P

def post (d : DCom) : Assertion :=
  match d with
  | dc_skip P => P
  | dc_seq _ d₂ => post d₂
  | dc_asgn _ _ Q => Q
...
def postcondition_from (dec : Decorated) : Assertion :=
  match dec with
...
def verification_conditions (P : Assertion) (d : DCom) : Prop :=
  match d with
  | dc_skip Q =>
      P ->> Q
  | dc_seq d₁ d₂ =>
  ...
  | dc_if b P₁ d₁ P₂ d₂ Q =>
      ...
      ∧ ((fun st => P st ∧ beval st (b_not b)) ->> P₂)
      ∧ (post d₁ ->> Q) ∧ (post d₂ ->> Q)
      ∧ verification_conditions P₁ d₁
      ∧ verification_conditions P₂ d₂
  | dc_while b Q d R =>
      ...
      ∧ ((fun st => post d st ∧ beval st (b_not b)) ->> R)
...
def verification_conditions_from (dec : Decorated) : Prop :=
  match dec with
